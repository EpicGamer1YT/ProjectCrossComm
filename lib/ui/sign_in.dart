...
class SignInState extends State<SignIn> {
          ...
          switch (snapshot.connectionState) {
            default:
              if (snapshot.hasError)
                ...
                return new Scaffold(
                  appBar: AppBar(
                    title: new Text("Welcome to Lingua!"),
                  ),
                  body: new ListView(
                    children: <Widget>[
                      new Column(
  ...
  TextFormField EmailField() {
    return TextFormField(
      ...
      decoration: InputDecoration(
          hintText: "Email", icon: Icon(Icons.email), fillColor: Colors.grey),
    );
  }
  ...
  Future<bool> _getUser() async {
    ...
    if (user != null) {
      torf = true;
      final FirebaseDatabase db = FirebaseDatabase();
      DataSnapshot versionCodeData = await db
          .reference()
          .child(Platform.isIOS ? "versionApple" : "versionAndroid")
          .once();
